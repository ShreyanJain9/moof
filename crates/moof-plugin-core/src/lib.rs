...
impl moof_core::Plugin for CorePlugin {
    ...
    fn register(&self, heap: &mut Heap) {
        ...
        // [env bind: name to: val] is the ONLY env handler that
        // stays native. it mutates the bindings Table — and Tables
        // are immutable from moof's side, so this can't be a
        // defmethod. everything else (at:, has?:, names, count,
        // typeName, describe, new, new:, walk:, walkTo:, union:)
        // lives in lib/kernel/namespace.moof now, where it's
        // moof-side, live-editable, and shorter.
