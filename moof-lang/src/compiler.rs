...
/// the 6 kernel forms and the syntactic desugaring (%send, %dot, %object-literal).
///
/// Tail position is tracked to emit OP_TAIL_APPLY for tail call optimization.
/// The bytecode is the canonical form -- what gets serialized.

use moof_fabric::{Value, HeapObject};
use moof_fabric::Heap;
use crate::opcodes::*;

/// A compiled bytecode chunk: code bytes + constant pool.
/// Stored in the heap as an Object with `code` (Bytes) and `constants` (list) slots.
...
impl BytecodeChunk {
    /// Store this chunk in the heap as an Object with code (Bytes) and constants (list) slots.
    pub fn store_in(self, heap: &mut Heap) -> u32 {
        let code_id = heap.alloc(HeapObject::Bytes(self.code));
        let constants_list = heap.list(&self.constants);
        let code_sym = heap.intern("code");
        let constants_sym = heap.intern("constants");
        let obj = heap.alloc(HeapObject::Object {
            parent: Value::Nil,
            slots: vec![
                (code_sym, Value::Object(code_id)),
                (constants_sym, constants_list),
            ],
            handlers: Vec::new(),
        });
        obj
...
impl Compiler {
    ...
    fn compile_vau_with_source(&mut self, heap: &mut Heap, args: &[Value], source_override: Value) -> Result<(), String> {
        ...
        let body_id = body_chunk.store_in(heap);
