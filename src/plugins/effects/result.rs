...
pub fn register(heap: &mut Heap) {
    ...
    native(heap, ok_id, "then:", |heap, receiver, args| {
        ...
        schedule_continuation(heap, f, val, false)
    });
    ...
    native(heap, ok_id, "map:", |heap, receiver, args| {
        ...
        let val_sym = heap.intern("value");
        let val = heap.get(id).slot_get(val_sym).unwrap_or(Value::NIL);
        schedule_continuation(heap, f, val, true)
    });
    ...
    native(heap, err_id, "recover:", |heap, receiver, args| {
        ...
        schedule_continuation(heap, f, msg, false)
    });
...
/// drains. Shared between Ok#then:, Ok#map:, Err#recover:, and
/// any other "call user code from native" path.
fn schedule_continuation(heap: &mut Heap, f: Value, val: Value, wrap_ok: bool) -> Result<Value, String> {
